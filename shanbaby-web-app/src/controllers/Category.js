...
export const insertCategory = (files, data) => {
  ...
  return new Promise((resolve, reject) => {
    return axios
      .post(`${Config.host}${Config.port}/category/insert`, formdata)
      .then((result) => {
...
export const updateCategory = (files, data, newImage) => {
  ...
  return new Promise((resolve, reject) => {
    return axios
      .patch(
        `${Config.host}${Config.port}/category/update/${data._id}`,
        formdata
      )
      .then((result) => {
        resolve({ code: 200, message: result.data.message });
...
export const deleteCategory = (id, token, type) => {
  ...
  return new Promise((resolve, reject) => {
    return axios
      .delete(`${Config.host}${Config.port}/category/delete/${id}`, {
        data: requestData,
      })
      .then((result) => {
        resolve({ code: 200, message: result.data.message });
