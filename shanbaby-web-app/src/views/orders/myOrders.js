...
class MyOrders extends Component {
  ...
  renderOrdersTable = (item) => {
    return (
      <tr key={item._id}>
        ...
        <td>
          {item.shipped ? (
            <span className="badge badge-pill badge-success">Shipped</span>
          ) : (
        ...
        <td>
          {item.deleteRequest ? (
            <span className="badge badge-pill badge-warning">
              Cancel Order Request Sent
            </span>
          ) : (
            <button
              className="btn btn-danger btn-sm px-2 mr-2"
              onClick={() => this.onClickDeleteRequest(item)}
            >
              Cancel Order
            </button>
          )}
  ...
  showOrderDetails = (products) => {
    ...
    return (
      <div
        className="card"
        style={{ width: "300px", overflow: "scroll", height: "300px" }}
      >
    ...
    return (
      <div key={product.id}>
        <div className="card-body">
          ...
          <p className="card-link" style={{ color: "blue" }}>
            LKR: {product.price}
          </p>
        </div>
      </div>
  ...
  onClickDeleteRequest = (item) => {
    ...
    if (item.shipped == true) {
      Config.setErrorToast("Cannot Delete Shipped Orders!");
    } else {
      updateOrder({
        id: item._id,
