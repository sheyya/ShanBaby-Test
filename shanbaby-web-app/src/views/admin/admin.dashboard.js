...
import { getCounts, getRevenue } from "../../controllers/Common";
...
class Dashboard extends Component {
  ...
  loadRevenue = () => {
    getRevenue()
      .then((result) => {
        // console.log(result);
        this.setState({
          revenue_labels: result.map((i) =>
            moment(new Date(i.date)).format("MMM DD")
          ),
  ...
  render() {
    const { counts, revenue_data, revenue_labels, orders } = this.state;
            ...
            <div className="row mx-1">
              <div className="col-12 px-0">
                <h5 className="text-dark bold-normal py-2 bg-white shadow-sm px-2 mt-3 rounded">
                  {this.setGreeting()}{" "}
                  <span className="small bold-normal text-muted">
                    {user.fname}
                  </span>
                </h5>
              </div>
              {count_data.map((item, i) => (
                <div
                  key={i}
                  className={`col-lg-3 col-md-3 col-sm-4 col-6 pl-0 pr-2`}
                >
                  <div className={cardstyle}>
                    <div className="pl-3 pr-0 my-auto">
                      <img
                        src={`images/default/admin.${item.name}.png`}
                        className="sidebar-image"
                      ></img>
                    </div>
                    <div className="my-auto">
                      <h6 className="text-secondary bold-normal pr-2">
                        {item.name}
                      </h6>
                      <h3 className="text-dark bold-normal pr-2">
