...
class Cart extends Component {
  ...
  componentDidMount() {
    this.props.getCart &&
      this.props
        .getCart(this.props.auth.user.id)
        .then((result) => {
                  ...
                  <div className="proceed-checkout">
                    <ul>
                      <li className="subtotal">
                        Sub-Total{" "}
                        <span>LKR {Config.calculate_full_total(cart)}</span>
                      </li>
                      <li className="subtotal">
                        Discount{" "}
                        <span>LKR {Config.calcualte_discount(cart)}</span>
                      </li>
                      <li className="subtotal">
                        <b>Total</b>{" "}
      ...
      <tr key={product._id}>
        ...
        <td>
          <Link to={`/product/${product._id}`}>
            <h5 className="form-label-table my-2 click">
              {product.name} -{" "}
