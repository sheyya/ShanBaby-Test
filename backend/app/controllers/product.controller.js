...
exports.update = (req, res, next) => {
  ...
  Product.findOne({ _id: id }, (err, found_product) => {
    ...
    if (!found_product) {
      ...
      //if name changed
      if (req.body.name) {
        found_product.name = req.body.name;
      }

      //if price changed
      if (req.body.price) {
        found_product.price = req.body.price;
      }

      //if description changed
      if (req.body.description) {
        found_product.description = req.body.description;
      }

      //if brand changed
      if (req.body.brand) {
        found_product.brand = req.body.brand;
      }

      //if discount changed
      if (req.body.discount) {
        found_product.discount = req.body.discount;
      }
      ...
      //if sizes changed
      if (req.body.sizes) {
        found_product.sizes = JSON.parse(req.body.sizes);
      }

      //if sizes changed
      if (req.body.tags) {
        found_product.tags = JSON.parse(req.body.tags);
      }

      //if colors changed
      if (req.body.colors) {
        found_product.colors = JSON.parse(req.body.colors);
      }
